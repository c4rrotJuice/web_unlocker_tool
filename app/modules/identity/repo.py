...
class IdentityRepository:
    ...
    async def _fetch_single(self, resource: str, *, user_id: str, order: str | None = None) -> dict[str, object] | None:
        ...
        if order:
            params["order"] = order
        response = await self.supabase_repo.get(resource, params=params, headers=self.supabase_repo.headers(include_content_type=False))
    ...
    async def fetch_profile(self, user_id: str) -> dict[str, object] | None:
        return await self._fetch_single("user_profiles", user_id=user_id)

    async def fetch_preferences(self, user_id: str) -> dict[str, object] | None:
        return await self._fetch_single("user_preferences", user_id=user_id)

    async def fetch_entitlement(self, user_id: str) -> dict[str, object] | None:
        return await self._fetch_single("user_entitlements", user_id=user_id)

...
function mapHttpError(status, fallbackCode = ERROR_CODES.NETWORK_ERROR) {
...
export function createApiClient({ baseUrl = API_ORIGIN, fetchImpl = globalThis.fetch, getAccessToken = async () => null, } = {}) {
    ...
    async function request(path, options = {}) {
        ...
        return createErrorResult(mapHttpError(response.status, fallbackCode), `Request failed with ${response.status}.`, undefined, parsed, { status: response.status });
    }
    return {
        request,
        ...
        getAuthAttemptStatus({ attemptId, attemptToken }) {
            return request(ENDPOINTS.AUTH_HANDOFF_ATTEMPT_STATUS.replace("{attempt_id}", encodeURIComponent(attemptId)), {
                headers: {

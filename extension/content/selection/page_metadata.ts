function walkElements(root: any, visit: (node: any) => void) {
...
function readMeta(documentRef: any, match: (name: string, property: string) => boolean) {
  let found = "";
...
function readLink(documentRef: any, relValue: string) {
  let found = "";
  walkElements(documentRef?.head || documentRef?.documentElement || documentRef?.body || null, (node) => {

import { computePillPosition } from "../selection/position.ts";
import { createSelectionMenuButton } from "./selection_menu_button.ts";
...
function createContainer(documentRef: any) {
...
function createPanel(documentRef: any) {
...
export function createSelectionActionPill({
  ...
  let visible = false;
  let currentPosition = null as any;
  let lastMessage = "";
  ...
  function setButtons(actions: any[] = []) {
    menu.innerHTML = "";
  ...
  function updatePosition(rect: any) {
  ...
  function setMessage(message: string) {
    lastMessage = message || "";
    const copyButton = Array.from(menu.children || []).find(
      (node: any) => node.getAttribute?.("data-selection-action") === "copy",
    );
    if (copyButton) {
  ...
  function flash(message: string, duration = 1200) {
    if (resetTimer) {
  ...
  function isInsidePill(target: any) {

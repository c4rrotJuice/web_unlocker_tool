...
export function normalizeCapabilitySurface({ auth = null, bootstrap = null } = {}) {
    ...
    if (!usageItems.length) {
        usageItems.push({ label: "Tier", value: tier }, { label: "Styles", value: String(citationStyles.length || CITATION_STYLES.length) }, { label: "Projects", value: String(Array.isArray(taxonomy.recent_projects) ? taxonomy.recent_projects.length : 0) }, { label: "Tags", value: String(Array.isArray(taxonomy.recent_tags) ? taxonomy.recent_tags.length : 0) });
    }
    return {
        auth: authState,

...
function mapRuntimeFailure(messageText, requestId) {
    ...
    if (/Extension context invalidated/i.test(normalized)) {
        return createErrorResult(ERROR_CODES.INVALID_CONTEXT, "Extension context invalidated. Reload the page and try again.", requestId);
    }
    return createErrorResult(ERROR_CODES.UNEXPECTED_ERROR, normalized || "Runtime message failed.", requestId);
}
export function sendRuntimeMessage(chromeApi, message) {
    ...
    if (!chromeApi?.runtime?.sendMessage) {
        return Promise.resolve(createErrorResult(ERROR_CODES.NOT_IMPLEMENTED, "chrome.runtime.sendMessage is unavailable.", message.requestId));
    }
    return new Promise((resolve) => {
        try {
            chromeApi.runtime.sendMessage(message, (response) => {
                ...
                if (!response) {
                    resolve(createErrorResult(ERROR_CODES.UNEXPECTED_ERROR, "No response received from background.", message.requestId));
                    return;
                }

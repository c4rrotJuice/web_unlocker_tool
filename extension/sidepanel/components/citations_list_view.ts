...
function summarizeAuthors(source: any = {}) {
...
export function createCitationsListView(options: any = {}) {
  const {
  ...
  function render(nextCitations = citations, expandedId = selectedCitationId) {
    ...
    for (const citation of nextCitations) {
      ...
      const previewText = getCitationPreviewText(
        citation,
        normalizeCitationStyle(citation.style || "apa"),
        normalizeCitationFormat(citation.format || "bibliography"),
      ) || citation.quote_text || citation.excerpt || "No citation text available.";

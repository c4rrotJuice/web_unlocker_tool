...
from app.services.free_tier_gating import current_week_window, doc_is_archived_for_free
...
def test_week_window_helpers_archive_behavior():
    ...
    assert doc_is_archived_for_free(archived_created_at, now) is True
    assert doc_is_archived_for_free(current_created_at, now) is False
...
def test_extension_selection_free_doc_limit_returns_editor_redirect(monkeypatch):
    ...
    monkeypatch.setattr(extension, "_count_docs_in_current_week", fake_count)
